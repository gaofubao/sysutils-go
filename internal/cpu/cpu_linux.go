...
func TimesWithContext(ctx context.Context, percpu bool) ([]TimesStat, error) {
	filename := pkg.HostProc("stat")
	var lines = []string{}
	if percpu {
		statlines, err := pkg.ReadLines(filename)
		...
		lines, _ = pkg.ReadLinesOffsetN(filename, 0, 1)
	}
...
func sysCPUPath(cpu int32, relPath string) string {
	return pkg.HostSys(fmt.Sprintf("devices/system/cpu/cpu%d", cpu), relPath)
}
...
func finishCPUInfo(c *InfoStat) error {
	...
	if len(c.CoreID) == 0 {
		lines, err = pkg.ReadLines(sysCPUPath(c.CPU, "topology/core_id"))
		if err == nil {
			c.CoreID = lines[0]
		}
	}
	...
	lines, err = pkg.ReadLines(sysCPUPath(c.CPU, "cpufreq/cpuinfo_max_freq"))
...
func InfoWithContext(ctx context.Context) ([]InfoStat, error) {
	filename := pkg.HostProc("cpuinfo")
	lines, _ := pkg.ReadLines(filename)
...
func CountsWithContext(ctx context.Context, logical bool) (int, error) {
	if logical {
		...
		procCpuinfo := pkg.HostProc("cpuinfo")
		lines, err := pkg.ReadLines(procCpuinfo)
		if err == nil {
			for _, line := range lines {
		...
		if ret == 0 {
			procStat := pkg.HostProc("stat")
			lines, err = pkg.ReadLines(procStat)
			if err != nil {
				return 0, err
			}
			for _, line := range lines {
	...
	for _, glob := range []string{"devices/system/cpu/cpu[0-9]*/topology/core_cpus_list", "devices/system/cpu/cpu[0-9]*/topology/thread_siblings_list"} {
		if files, err := filepath.Glob(pkg.HostSys(glob)); err == nil {
			for _, file := range files {
				lines, err := pkg.ReadLines(file)
