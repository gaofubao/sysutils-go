...
func IOCountersByFileWithContext(ctx context.Context, pernic bool, filename string) ([]IOCountersStat, error) {
	lines, err := pkg.ReadLines(filename)
	if err != nil {
		return nil, err
	}
...
func ProtoCountersWithContext(ctx context.Context, protocols []string) ([]ProtoCountersStat, error) {
	...
	filename := pkg.HostProc("net/snmp")
	lines, err := pkg.ReadLines(filename)
	if err != nil {
		return nil, err
	}
...
func FilterCountersWithContext(ctx context.Context) ([]FilterStat, error) {
	countfile := pkg.HostProc("sys/net/netfilter/nf_conntrack_count")
	maxfile := pkg.HostProc("sys/net/netfilter/nf_conntrack_max")

	count, err := pkg.ReadInts(countfile)
...
func ConntrackStatsWithContext(ctx context.Context, percpu bool) ([]ConntrackStat, error) {
	return conntrackStatsFromFile(pkg.HostProc("net/stat/nf_conntrack"), percpu)
}
	...
	for _, line := range lines {
		...
		if len(fields) == 17 && fields[0] != "entries" {
			statlist.Append(NewConntrackStat(
				pkg.HexToUint32(fields[CT_ENTRIES]),
				pkg.HexToUint32(fields[CT_SEARCHED]),
				pkg.HexToUint32(fields[CT_FOUND]),
				pkg.HexToUint32(fields[CT_NEW]),
				pkg.HexToUint32(fields[CT_INVALID]),
				pkg.HexToUint32(fields[CT_IGNORE]),
				pkg.HexToUint32(fields[CT_DELETE]),
				pkg.HexToUint32(fields[CT_DELETE_LIST]),
				pkg.HexToUint32(fields[CT_INSERT]),
				pkg.HexToUint32(fields[CT_INSERT_FAILED]),
				pkg.HexToUint32(fields[CT_DROP]),
				pkg.HexToUint32(fields[CT_EARLY_DROP]),
				pkg.HexToUint32(fields[CT_ICMP_ERROR]),
				pkg.HexToUint32(fields[CT_EXPECT_NEW]),
				pkg.HexToUint32(fields[CT_EXPECT_CREATE]),
				pkg.HexToUint32(fields[CT_EXPECT_DELETE]),
				pkg.HexToUint32(fields[CT_SEARCH_RESTART]),
			))
		}
	}
...
// move to pkg made other platform breaking. Need consider.
...
func (p *process) fillFromStatus() error {
	pid := p.Pid
	statPath := pkg.HostProc(strconv.Itoa(int(pid)), "status")
...
func processInet(file string, kind netConnectionKindType, inodes map[string][]inodeMap, filterPid int32) ([]connTmp, error) {
	...
	if strings.HasSuffix(file, "6") && !pkg.PathExists(file) {
