...
func PartitionsWithContext(ctx context.Context, all bool) ([]PartitionStat, error) {
	...
	filename := pkg.HostProc("self/mountinfo")
	...
	if err != nil {
		...
		useMounts = true
		filename = pkg.HostProc("self/mounts")
		lines, err = pkg.ReadLines(filename)
		if err != nil {
			return nil, err
		}
	...
	for _, line := range lines {
		...
		if useMounts {
			...
			if !all {
				if d.Device == "none" || !pkg.StringsHas(fs, d.Fstype) {
					continue
				}
			}
		} else {
			...
			if strings.HasPrefix(d.Device, "/dev/mapper/") {
				devpath, err := filepath.EvalSymlinks(pkg.HostDev(strings.Replace(d.Device, "/dev", "", -1)))
				if err == nil {
					d.Device = devpath
				}
			}
			...
			if d.Device == "/dev/root" {
				devpath, err := os.Readlink(pkg.HostSys("/dev/block/" + blockDeviceID))
				if err != nil {
					return nil, err
				}
...
func getFileSystems() ([]string, error) {
	filename := pkg.HostProc("filesystems")
	lines, err := pkg.ReadLines(filename)
	if err != nil {
		return nil, err
	}
...
func IOCountersWithContext(ctx context.Context, names ...string) (map[string]IOCountersStat, error) {
	filename := pkg.HostProc("diskstats")
	lines, err := pkg.ReadLines(filename)
	if err != nil {
		return nil, err
	}
	...
	for _, line := range lines {
		...
		if len(names) > 0 && !pkg.StringsHas(names, name) {
			continue
		}
...
func GetDiskSerialNumberWithContext(ctx context.Context, name string) string {
	...
	udevDataPath := pkg.HostRun(fmt.Sprintf("udev/data/b%d:%d", major, minor))
	...
	devicePath := pkg.HostSys(fmt.Sprintf("dev/block/%d:0/device", major))
...
func GetLabel(name string) string {
	...
	dmname_filename := pkg.HostSys(fmt.Sprintf("block/%s/dm/name", name))

	if !pkg.PathExists(dmname_filename) {
		return ""
	}
