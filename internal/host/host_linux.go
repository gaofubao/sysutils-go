...
func HostIDWithContext(ctx context.Context) (string, error) {
	sysProductUUID := pkg.HostSys("class/dmi/id/product_uuid")
	machineID := pkg.HostEtc("machine-id")
	procSysKernelRandomBootID := pkg.HostProc("sys/kernel/random/boot_id")
	switch {
	// In order to read this file, needs to be supported by kernel/arch and run as root
	// so having fallback is important
	case pkg.PathExists(sysProductUUID):
		lines, err := pkg.ReadLines(sysProductUUID)
		if err == nil && len(lines) > 0 && lines[0] != "" {
			return strings.ToLower(lines[0]), nil
		}
		fallthrough
	// Fallback on GNU Linux systems with systemd, readable by everyone
	case pkg.PathExists(machineID):
		lines, err := pkg.ReadLines(machineID)
	...
	default:
		lines, err := pkg.ReadLines(procSysKernelRandomBootID)
		if err == nil && len(lines) > 0 && lines[0] != "" {
			return strings.ToLower(lines[0]), nil
		}
	}
...
func BootTimeWithContext(ctx context.Context) (uint64, error) {
	return pkg.BootTimeWithContext(ctx)
}
...
func UsersWithContext(ctx context.Context) ([]UserStat, error) {
	...
	for i := 0; i < count; i++ {
		...
		user := UserStat{
			User:     pkg.IntToString(u.User[:]),
			Terminal: pkg.IntToString(u.Line[:]),
			Host:     pkg.IntToString(u.Host[:]),
...
func getLSB() (*LSB, error) {
	ret := &LSB{}
	if pkg.PathExists(pkg.HostEtc("lsb-release")) {
		contents, err := pkg.ReadLines(pkg.HostEtc("lsb-release"))
		if err != nil {
	...
	} else if pkg.PathExists("/usr/bin/lsb_release") {
...
func PlatformInformationWithContext(ctx context.Context) (platform string, family string, version string, err error) {
	...
	if pkg.PathExists(pkg.HostEtc("oracle-release")) {
		platform = "oracle"
		contents, err := pkg.ReadLines(pkg.HostEtc("oracle-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
		}

	} else if pkg.PathExists(pkg.HostEtc("enterprise-release")) {
		platform = "oracle"
		contents, err := pkg.ReadLines(pkg.HostEtc("enterprise-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
		}
	} else if pkg.PathExists(pkg.HostEtc("slackware-version")) {
		platform = "slackware"
		contents, err := pkg.ReadLines(pkg.HostEtc("slackware-version"))
		if err == nil {
			version = getSlackwareVersion(contents)
		}
	} else if pkg.PathExists(pkg.HostEtc("debian_version")) {
		if lsb.ID == "Ubuntu" {
			platform = "ubuntu"
			version = lsb.Release
			...
			if pkg.PathExists("/usr/bin/raspi-config") {
				platform = "raspbian"
			} else {
				platform = "debian"
			}
			contents, err := pkg.ReadLines(pkg.HostEtc("debian_version"))
			if err == nil && len(contents) > 0 && contents[0] != "" {
				version = contents[0]
			}
		}
	} else if pkg.PathExists(pkg.HostEtc("redhat-release")) {
		contents, err := pkg.ReadLines(pkg.HostEtc("redhat-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
			platform = getRedhatishPlatform(contents)
		}
	} else if pkg.PathExists(pkg.HostEtc("system-release")) {
		contents, err := pkg.ReadLines(pkg.HostEtc("system-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
			platform = getRedhatishPlatform(contents)
		}
	} else if pkg.PathExists(pkg.HostEtc("gentoo-release")) {
		platform = "gentoo"
		contents, err := pkg.ReadLines(pkg.HostEtc("gentoo-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
		}
	} else if pkg.PathExists(pkg.HostEtc("SuSE-release")) {
		contents, err := pkg.ReadLines(pkg.HostEtc("SuSE-release"))
		if err == nil {
			version = getSuseVersion(contents)
			platform = getSusePlatform(contents)
		}
		// TODO: slackware detecion
	} else if pkg.PathExists(pkg.HostEtc("arch-release")) {
		platform = "arch"
		version = lsb.Release
	} else if pkg.PathExists(pkg.HostEtc("alpine-release")) {
		platform = "alpine"
		contents, err := pkg.ReadLines(pkg.HostEtc("alpine-release"))
		if err == nil && len(contents) > 0 && contents[0] != "" {
			version = contents[0]
		}
	} else if pkg.PathExists(pkg.HostEtc("os-release")) {
		p, v, err := pkg.GetOSRelease()
		if err == nil {
			platform = p
			version = v
		}
...
func VirtualizationWithContext(ctx context.Context) (string, string, error) {
	return pkg.VirtualizationWithContext(ctx)
}

func SensorsTemperaturesWithContext(ctx context.Context) ([]TemperatureStat, error) {
	var temperatures []TemperatureStat
	files, err := filepath.Glob(pkg.HostSys("/class/hwmon/hwmon*/temp*_*"))
	if err != nil {
	...
	if len(files) == 0 {
		...
		files, err = filepath.Glob(pkg.HostSys("/class/hwmon/hwmon*/device/temp*_*"))
		if err != nil {
			return temperatures, err
		}
	}
	...
	if len(files) == 0 { // handle distributions without hwmon, like raspbian #391, parse legacy thermal_zone files
		files, err = filepath.Glob(pkg.HostSys("/class/thermal/thermal_zone*/"))
		if err != nil {
			return temperatures, err
		}
