...
func PidExistsWithContext(ctx context.Context, pid int32) (bool, error) {
	...
	if isMount(pkg.HostProc()) { // if /<HOST_PROC>/proc exists and is mounted, check if /<HOST_PROC>/proc/<PID> folder exists
		_, err := os.Stat(pkg.HostProc(strconv.Itoa(int(pid))))
